...
class primary_registration extends CI_model
	{
	...
	function show($id=0,$edit=1,$erro='')
		{
			...
			$sql = "select * from cep_submit_register_unit
						INNER JOIN register_unit ON ru_codigo = csru_unit
					where csru_protocolo = '$proto' and ru_ativo = 1 and csru_ativo = 1
					order by ru_name ";
			$rlt = $this->db->query($sql);
			...
			$sx .= '<tr>
						<th width="2%">#</th>
						<th width="60%">'.msg('ru_name').'</th>
						<th width="26%">'.msg('pais_nome').'</th>
						<th width="10%">'.msg('country_description').'</th>
						<th>-</th>						
					</tr>';
			$tot = 0;
	...
	function novo_form($proto)
		{
			...
			for ($r=0;$r < count($rlt);$r++)
				{
					$line = $rlt[$r];
					$op .= '<option value="'.$line['ru_codigo'].'">'.msg($line['ru_name']).'</option>'.cr();
				}
			$op .= '</select>'.cr();
			...
			/* Description */
			$sx .= '<div class="col-md-1 small text-right">'.cr();
			$sx .= msg('date').':'.cr();
			$sx .= '</div>'.cr();
			
			$sx .= '<div class="col-md-1">'.cr();
			$sx .= '<input type="string" id="register_p" value="" class="form-control">'.cr();
			$sx .= '</div>'.cr();
			
			$sx .= '<div class="col-md-2">';
			$sx .= '	<button type="button" class="btn btn-primary" id="register_p_post">'.msg("register_p_post").'</button>';
			$sx .= '</div>'.cr();
			
			$sx .= '</div> <!-- Row -->'.cr();
			
			$sx .= '<input type="hidden" id="proto_register_p" value="'.$proto.'">'.cr();
			$sx .= '<input type="hidden" id="proto_check_register_p" value="'.checkpost_link($proto).'">'.cr();
			$sx .= '<input type="hidden" id="link_register_p" value="'.base_url('index.php/ajax/action/register_p').'">'.cr();
